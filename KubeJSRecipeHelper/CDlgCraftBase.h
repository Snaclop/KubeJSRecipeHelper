...
//
// 网格完全由代码在对话框客户区上直接绘制，不依赖 .rc 里的任何控件：
//   - 左键点击某个格子，模态弹出 CDlgSelect 选择物品与数量；
//   - 右键点击某个格子，清空该格；
//   - 点“确定”后由子类按格子内容生成对应的 KubeJS 脚本。
//
// 子类只需要给出对话框模板，并实现 BuildRecipeScript()（有序 / 无序的区别就在那里）。
class CDlgCraftBase : public CDialogEx
{
	DECLARE_DYNAMIC(CDlgCraftBase)
...
protected:
	...
	// nIDTemplate 由子类传进来（各自对应 IDD_DLGSHAPED / IDD_DLGSHAPELESS）
	CDlgCraftBase(UINT nIDTemplate, CWnd* pParent = nullptr);
	virtual ~CDlgCraftBase();
	...
	virtual CString BuildRecipeScript() const = 0;
	// 提示文字，子类可以换掉
	virtual LPCTSTR GetHintText() const;
	// 点“确定”前的检查，返回 FALSE 时通过 strError 带出提示
